...
package utils
{
	...
	public class StringUtils {
		...
		/**
		 * Creates a timestamp which follows a predefined structure. This is mainly used to tag the current collection or the current item and prefix or suffix it to the name of the file which will be sent to the server.
		 * 
		 */
		public static function get_timestamp():String
		{
			var d:Date = new Date();
			var datestr:String= d.getFullYear().toString() + "_" 	// year_ 
		...
		*/
		...
		/**
		 * Determines whether the specified string contains any instances of <code>p_char</code>
		*
		*	@param p_string The string.
		*
		...
		*/
		...
		/**
		 * eturns the specified string in reverse character order.
		*
		...
		*/
		...
		/**
		*	
		 * turns a string truncated to a specified length with optional suffix
		*
		*	@param p_string The string.
		*
		...
		*/
		...
		/**
		 * Searches a String for a pattern, specified by another String. If this pattern is found, the functions returns true else false.
		 * 
		 * @param toCompare THe String to search through
		 * @paran pattern The pattern tobe found
		 * @returns Boolean false if not found true if found
		 * @langversion ActionScript 3.0
		 * @playerversion Flash 9.0
		 * @tiptext
		 */
		public static function checkStringPattern(toCompare:String, pattern:String):Boolean
		{
			return toCompare.indexOf(pattern)>-1;
		}
		
		
		/**
		 * Find specific words in a String. The comparison is done via levenshtein-distance, so typos can't kill the search. The Strings will be splitted into arrays of strings without space characters. It will return true only if all words were found in the string, otherwise it returns false.
		 * 
		 * @param toCompare THe String to search through
		 * @param match The words to search
		 * @paran max_distance levenshtein maximal distance to allow typo errors
		 * @returns Boolean false if not found true if found
		 * @langversion ActionScript 3.0
		 * @playerversion Flash 9.0
		 * @tiptext
		 */
		public static function findWords(toCompare:String, match:String, max_distance:uint):Boolean
		{
			
			var split:Array = toCompare.split(" ");
			var search:Array = match.split(" ");
			
			var foundCount:uint = 0;
			//trace("[StringUtils]: Find " + search.length + " words ");
			for(var i:uint = 0; i<split.length;i++)
			{

				for(var j:uint=0;j<search.length;j++)
				{
					//trace("[StringUtils]: compare " + split[i] + " with "+ search[j] + " (distance = "+ max_distance+ ")");
					//trace(" => "+editDistance(split[i],search[j]));
					if(editDistance(split[i],search[j])<=max_distance)
					{
						//trace("=> found a word");
						foundCount++;	
					}
			...
			{
				//trace("[StringUtils]: found all words");
				return true;
			}
			return false;
		}
		...
		public static function createRandomWord(numchars:uint):String
		{
			var s:String="";
			for(var i:uint=0;i<numchars;i++)
			{
				s+= String.fromCharCode(MathUtils.randomRange(65,90));
			}
			return s;
		}
