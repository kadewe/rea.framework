	...
	<xsl:template name="description">
		...
		<xsl:if test="string-length($asCustomsText) &gt; 0">
			...
			<xsl:value-of select="$asCustomsText"/>
				    ...
				    <xsl:if test="customs/author">
				<p><font color="#00FF00">Author: </font><xsl:value-of select="author/." /></p>
			</xsl:if>
			<xsl:if test="customs/bug">
				<p><font color="#FF00FF">BUG: </font><xsl:value-of select="customs/bug/." /></p>
			</xsl:if>
			<xsl:if test="customs/todo">
				<p><font color="#0000FF">To do: </font><xsl:value-of select="customs/todo/." /></p>
			</xsl:if>
